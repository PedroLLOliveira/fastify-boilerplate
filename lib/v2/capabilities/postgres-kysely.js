...
// Antes existiam dois pools de conexão no mesmo app (achado 13): este
// plugin abria o próprio \`pg.Pool\` e decorava \`fastify.dbPool\`, mas nenhum
// repositório o usava — todos importam o singleton \`db\` de database.ts, que
// já tem seu próprio pool interno via PostgresDialect. O plugin agora só
// referencia esse singleton: testa a conexão no boot (fail-fast) e registra
// o \`db.destroy()\` no shutdown gracioso do app (SIGTERM/SIGINT → app.close()
// → este onClose).
const dbLifecycleContent = `import fp from 'fastify-plugin';
...
// Sem isso, um cliente ocioso do pool que perde a conexão (ex.: Postgres
// reiniciando) derruba o processo inteiro com um 'error' event não tratado
// — o pool do node-postgres não segura esse erro sozinho.
pool.on('error', (err) => {
...
export const postgresKyselyCapability = {
  ...
  files: [
    ...
    { path: 'src/plugins/db-lifecycle.ts', content: dbLifecycleContent },
  ...
  appFragment: {
    // O plugin agora referencia o singleton `db` diretamente (ele mesmo
    // chama loadEnv() dentro de database.ts), então o app.ts não precisa
    // mais de `const env = loadEnv()` nem de importar loadEnv só para isso.
    imports: ["import dbLifecycle from './plugins/db-lifecycle.js';"],
    needsEnv: false,
    registration: 'await app.register(dbLifecycle);'
  }
};
