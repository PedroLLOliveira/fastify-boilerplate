...
// Vira uma função (em vez de string fixa) na Fase 5: precisa aceitar o
// fragmento de uma capability de plataforma (ex.: CORS) sem reescrever o
// error handler/not-found handler para cada uma — mesma ideia de
// `architectures/modular-persisted.js::buildAppTsContent`, só que para a
// variante sem persistência. Chamada sem argumento (perfil `modular` puro),
// gera exatamente o mesmo `app.ts` de antes da Fase 5.
export function buildAppTsContent(fragment = {}) {
  const imports = fragment.imports?.length ? fragment.imports.join('\n') + '\n' : '';
  const lifecycle = fragment.registration ? `\n  // Lifecycle\n  ${fragment.registration}\n` : '';
...
describe('Users API flow', () => {
  ...
  test('POST /users returns 400 on invalid body', async () => {
    const res = await app.inject({ method: 'POST', url: '/users', payload: {} });
    expect(res.statusCode).toBe(400);
    expect(res.json().error).toBe('Bad Request');
  });

  test('POST /users returns 409 on conflict', async () => {
    const res = await app.inject({ method: 'POST', url: '/users', payload: { name: 'Alice', email: 'alice@test.com' } });
