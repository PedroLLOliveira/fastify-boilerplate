...
export const repo = (db: DB) => ({
  ...
  delete: async (id: string) => {
    // delete lança se não existir — padronizamos retorno em número
    try {
 ...
 * - v6 Querying - Basics/Finders (CRUD): https://sequelize.org/docs/v6/core-concepts/model-querying-basics/
 * - v6 Querying - Finders: https://sequelize.org/docs/v6/core-concepts/model-querying-finders/
 * - v7 (alpha) Models & Tables / Defining models: https://sequelize.org/docs/v7/models/defining-models/
 */

import type { Sequelize, ModelStatic, Model } from 'sequelize';
import { defineUser } from './model';
 ...
 * - Queries (lean, findByIdAndUpdate): https://mongoosejs.com/docs/api/query.html
...
export const repo = (_db: unknown) => ({
  ...
  delete: (id: string) => {
    const b = store.data.length;
    store.data = store.data.filter((u) => u.id !== Number(id));
    return b - store.data.length;
  },
});
`;
}
