...
import { entityTemplate } from './entity.ts.js';
import { repoPortTemplate } from './repoPort.ts.js';
import { usecasesTemplate } from './usecases.ts.js';
import { repoImplTemplate } from './repoImpl.ts.js';
...
export async function generate({ root, ext, orm }) {
  ...
  await fsp.writeFile(path.join(infra, 'http', `users.controller.${ext}`), controllerTemplate(orm), 'utf8');
}
