...
test('V2 E2E Eval - Profile Modular PG Sequelize', async (t) => {
  ...
  try {
    ...
    console.log('[E2E-SEQ] Running build...');
    await runCommand('npm', ['run', 'build'], projectPath);

    console.log('[E2E-SEQ] Running native tests...');
    ...
    console.log('[E2E-SEQ] Eval finished perfectly!');
  } catch (err) {
    console.error('Eval failed with error:', err);
    throw err;
  } finally {
    // Teardown docker
    console.log('[E2E-SEQ] Tearing down Docker...');
